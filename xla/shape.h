...
class Shape {
 public:
  ...
  // to the default (empty) state for the new type.
  ...
  int64_t dimensions_minor(int index) const {
    CHECK(has_layout());
    const auto* const state = array_state();
  ...
  bool has_layout() const {
    return IsArray() && array_state()->layout != std::nullopt;
  }
  ...
  void clear_dynamic_dimensions() {
    if (IsArray()) {
      ...
      for (auto& subshape : tuple_state()->tuple_shapes) {
  ...
  void Clear() { set_element_type(PRIMITIVE_TYPE_INVALID); }
  ...
  static H Hash(H h, const Shape& s) {
    if (s.IsTuple()) {
      for (const Shape& subshape : s.tuple_state()->tuple_shapes) {
        h = Shape::Hash<H, kIsLayoutSensitive>(std::move(h), subshape);
      }
      return H::combine(std::move(h), s.tuple_shapes_size());
    }
    if (s.IsArray()) {
      h = H::combine(std::move(h), s.element_type_, s.array_state()->dimensions,
                     s.array_state()->dynamic_dimensions);
      if (kIsLayoutSensitive) {
        h = H::combine(std::move(h), s.array_state()->layout);
      }
      return std::move(h);
    }
 ...
 private:
  ...
  //     fields that don't match the shape's element type.
  ...
  //     fields that are irrelevant to a given shape category.
  ...
  // Convenience accessors for the state_ variant. Each accessor returns a
  // pointer to the corresponding state struct, or nullptr if the shape is not
  ...
  ArrayState* array_state() { return std::get_if<ArrayState>(&state_); }
  const TupleState* tuple_state() const {
    return std::get_if<TupleState>(&state_);
  }
  TupleState* tuple_state() { return std::get_if<TupleState>(&state_); }
};
...
class ProgramShape {
 public:
  ...
  // The following methods mirror the protobuf generated code interface for
  // the message ProgramShapeProto. This enabled easy migration of this data
 ...
 private:
  // The shapes of the parameters of the computation represented by this
  // object.
  std::vector<Shape> parameters_;

  // The names of the parameters of the computation represented by this
  // object.
