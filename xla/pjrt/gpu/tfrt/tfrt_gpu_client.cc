...
void EnqueueWork(xla::WorkerThread* thread, absl::AnyInvocable<void()> callee) {
  thread->Schedule(std::move(callee));
}
...
class TfrtGpuAsyncHostToDeviceTransferManager
 ...
 public:
  ...
  absl::Status TransferLiteralToBuffer(
    ...
    EnqueueWork(h2d_thread_.get(), std::move(transfer_h2d));
    return absl::OkStatus();
  }
  ...
  absl::Status TransferRawDataToSubBuffer(
    ...
    EnqueueWork(h2d_thread_.get(), std::move(copy_to_gpu));
    return absl::OkStatus();
  }
